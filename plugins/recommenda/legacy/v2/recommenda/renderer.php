...
function execute_interests_form($interests = array()) {
    ...
    $html .= html_writer::tag('h3', 'Bem vindo(a) ao ambiente MOOC, ' . $USER->firstname . ' ' . $USER->lastname . '!' . html_writer::tag('br', ''));
    $html .= html_writer::tag('h4', 'Receba recomendações personalizadas com base nos itens de seu interesse.' . html_writer::tag('br', ''));
    ...
    foreach($all_tags as $key => $tag){
        ...
        $col = 'col';
        if($boolLastKey && $countItens != 3){
            $col = 'col-md-' . $countItens * 4;
        }
        $html .= '<div class="'.$col.' checkbox">';
        $checked = '';
...
foreach ($final_array as $flag_key => $courseid) 
{
    ...
    {
        $temp_html .= '<div class="carousel-item col-12 col-sm-6 col-md-4">';
    }
    ...
    if(empty($course_formatted->get_course_overviewfiles())){
        ...
        $content .= html_writer::start_div('courseimage img-fluid mx-auto min-dimensions', array('id'=> 'background'.$valid_counter.''));

        $content .= html_writer::start_div('courseimage-overlay img-fluid mx-auto min-dimensions');

        setrawcookie($valid_counter, $img_url);
        //print_r2('<br></br><br></br><br></br>Cookie '.$valid_counter.' criado! Conteúdo: '.$img_url.' Cookie: '.$_COOKIE[$valid_counter]);
    }
    else
...
@element .block_recommenda and (min-width: 720px) and (max-width: 991px) {
